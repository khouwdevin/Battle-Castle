...
public class HeroesArcher extends Heroes{

    public static final float kill_cavalry = 0.4f;
    public static final float kill_infantry = 0.1f;

    public HeroesArcher(int heroes_type, int[] army_type, int[] troops) {
                ...
                if (p.heroes_type == 1) {
                    if (p.army_type[j] == 1) {
                        if (e.army_type[i] == 1) {
                            ...
                            e.casualties += (int) ((p.troops[j] + attack_boost * p.troops[i] * heroes_send) * HeroesArcher.kill_cavalry);
                        } else {
                            e.casualties += (int) ((p.troops[j] + attack_boost * p.troops[i] * heroes_send) * HeroesArcher.kill_infantry);
                        }
